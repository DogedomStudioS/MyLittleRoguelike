...
func split(iterations = 1, minimum_size = Vector2(1, 1), variance = 3.0):
	...
	var tooSmall = (
		bounds.position.x < 1
		or bounds.position.y < 1
		or bounds.size.x / 2 < minimum_size.x
		or bounds.size.y / 2 < minimum_size.y
	)
	if tooSmall:
		return
	...
	var minimum_split = (
		floor(bounds.size[split_direction] / 2.0)
		- floor(bounds.size[split_direction] * variance)
	)
